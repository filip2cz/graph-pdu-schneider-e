...
def graph_data(input_file_name, output_file_name, size = (20, 10), dpi=80, smoothing=20, secondary_smoothing=0, title = None, font_size=20, normalize=True):
    ...
    for ind in data.columns:
        col = data[ind]
        if ind != 'Date':
            data[ind]=pd.to_numeric(col, errors="coerce")
    data.dropna(inplace=True, subset=["Pwr.kW", "Pwr Max.kW", "Ph I.A", "Ph I Max.A"], axis=0)
    ...
    if secondary_smoothing != 0:
        for ind in data.columns:
            ...
            if ind != 'Date':
                data[ind]=col.rolling(smoothing).mean()
    fig, axs = plt.subplots(3, dpi=dpi, figsize=size)
    #plot data
    ...
    axs[2].plot(data["Date"], data["Energy.kWh"], label="Energy.kWh")
    #show legend / labels
    axs[0].legend()
    axs[1].legend()
    axs[2].legend()
